...
`build.sh` builds both images and pushes them. Override the destination repo and
...
- **`jaz-web`** compiles the SPA from `frontend/`, which is **not** in this repo.
  Build it with the Jaz source tree as context, e.g. copy `deploy/docker/` into a
  Jaz checkout (or run `build.sh` from there). `jaz-web` has no Jaz-version pin —
  it ships whatever `frontend/` builds.
